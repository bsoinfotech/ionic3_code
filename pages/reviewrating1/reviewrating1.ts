...
@IonicPage()
@Component({
  selector: 'page-reviewrating1',
  templateUrl: 'reviewrating1.html',
})
export class Reviewrating1Page {
problem:any;
pro_attitude:any;
qualityofwork:any;
ontimereporting:any;
...
//alert('hello');
		if(this.ontimereporting=='' || this.ontimereporting==undefined)
	    {
	        let toast = this.toastCtrl.create({
	            message: 'Provide Technician Rating',
	            duration: 3000,
	            position: 'bottom'
	        });
	        toast.present();
    ...
    */
		...
		this.storage.get('userid').then((userid) => {
		 let loading = this.loadingCtrl.create({ spinner: 'hide',
			...
			this.api.post('savetechrating',{ userid:userid,order_num:this.order_num,remark:this.remark,problem: this.problem, pro_attitude:this.pro_attitude, qualityofwork:this.qualityofwork,ontimereporting:this.ontimereporting,spid:spid,first:first,second:second,third:third,fourth:fourth}).subscribe((resp : any) => {
			loading.dismiss();
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad Reviewrating1Page');
  }
